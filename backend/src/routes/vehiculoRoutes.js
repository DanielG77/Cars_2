...
const validateVehiculo = [
    ...
    body('puertas').optional({ nullable: true }).isInt({ min: 2, max: 7 }).withMessage('Número de puertas no válido'),
    body('status').optional({ nullable: true }).isIn(['accepted', 'pending', 'rejected']).withMessage('Status no válido')
];
...
router.patch('/:id/status', vehiculoController.patchStatus);
